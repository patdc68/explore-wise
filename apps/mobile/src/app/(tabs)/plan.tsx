...
import { ClayCard, ClaySurface, LoadingCard, PrimaryButton, SecondaryButton, ScreenSection, SectionHeader } from '@/components/ui/clay';
...
import { ADD_STOP_CATEGORIES, addUserStage, areRequiredStagesComplete, filterCandidatesForStage, finalizeItinerary, isPlaceAvailableForStage, mapStops, MAX_ITINERARY_STOPS, navigationUrl, remainingBudget, removeUserStage, selectStop, selectedPlaceExclusionKey, stageActivityFocus, stageIndexAfterRemoval, stageOrigin, stageOriginKey, type AddStopCategoryId, type ItineraryState } from '@/services/itinerary';
...
export default function PlanScreen() {
  ...
  return <Shell theme={theme} resetScrollKey={executionPresentationKey} bottomAction={execution?.status === 'planned' ? <PrimaryButton label="Start itinerary" accessibilityLabel="Start itinerary" labelNumberOfLines={0} fullWidth style={styles.plannedStart} onPress={() => executionStore.dispatch({ type: 'start', itineraryId: execution.itineraryId })} /> : undefined}>{execution ? <ItineraryProgress execution={execution} /> : <Heading eyebrow="REVIEW YOUR PLAN" title="Check the details." subtitle={notice ?? 'Edit any stop before you finalize.'} />}
    ...
    {!execution ? <>{canAdd ? <SecondaryButton label="+ Add another stop" accessibilityLabel="Add another stop" onPress={() => setScreen('add-category')} /> : editable ? <ThemedText type="small" themeColor="textSecondary">Maximum of {MAX_ITINERARY_STOPS} stops reached. Remove a manually added stop to add another.</ThemedText> : null}{!complete ? <ThemedText type="small" themeColor="textSecondary">Required stages are incomplete. Add the missing stops before finalizing.</ThemedText> : editable ? <PrimaryButton label="Finalize itinerary" onPress={() => { const finalized = executionStore.finalize(randomUUID(), finalizeItinerary(state)); if (!finalized) return; setState(finalized.itinerary); clearAlternatives(); setScreen('finalized'); setNotice(null); }} /> : null}<View style={styles.actions}>{editable ? <SecondaryButton label="Customize" onPress={() => { setStageIndex(0); setScreen('guided'); }} /> : null}<StartOverAction onPress={clearPlan} /></View></> : execution.status === 'planned' ? <View style={styles.actions}><StartOverAction onPress={clearPlan} /></View> : null}</Shell>;
}
