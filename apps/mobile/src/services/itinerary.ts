...
export function navigationUrl(place: Pick<PricedNearbyPlace, 'latitude' | 'longitude' | 'name'>) { return `https://www.google.com/maps/dir/?api=1&destination=${encodeURIComponent(`${place.latitude},${place.longitude}`)}&destination_place_id=${encodeURIComponent(place.name)}`; }
