...
4. A verified mapping resolves an external category to an existing `ew_categories.code`. Unknown categories are rejected or marked for review according to importer configuration.
...
Required validation covers ingestion-run linkage, registered source, source place ID, name, country-code shape, coordinates, URLs when supplied, source timestamps when supplied, duplicate run identity, and verified category mapping. Failures are stored as objects with `code`, `field`, `message`, and an optional rejected value.

## Category mapping

Mappings live under `data/category-mappings/`. The Foursquare mapping registry is intentionally empty in this foundation milestone: no unverified taxonomy IDs or names are guessed. The fixture adapter injects a separate, clearly test-only mapping to exercise the pipeline.
...
Metro Manila is defined in `data/regions/metro-manila.ts` as `PH-NCR`, `PH`, `Metro Manila`, `Asia/Manila`, and `PHP`. These values are not defaults on `ew_places`.
