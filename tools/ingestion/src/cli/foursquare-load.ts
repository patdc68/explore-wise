...
const HISTORICAL_STAGING_ROWS = 5_150;
...
async function preflight(runId: string, state: LoaderState, database: Client): Promise<void> {
  ...
  const expectedTotal = HISTORICAL_STAGING_ROWS + Number(runRows);
  if (Number(staging) !== expectedTotal) throw new Error(`Preflight failed: staging history is not preserved (total ${staging}, run rows ${runRows}, expected historical baseline ${HISTORICAL_STAGING_ROWS}).`);
...
async function ensureRun(runId: string, database: Client): Promise<void> {
  await database.query(`insert into public.ew_ingestion_runs (id, source_id, region_code, status) values (${sqlLiteral(runId)}, ${sqlLiteral(SOURCE_ID)}, 'PH-NCR', 'running') on conflict (id) do update set status = 'running' where public.ew_ingestion_runs.source_id = excluded.source_id;`);
}

async function reconcileRun(runId: string, database: Client): Promise<void> {
...
select count(*) from public.ew_place_import_staging where ingestion_run_id = ${sqlLiteral(runId)} and (latitude not between 14.3472554 and 14.7853355 or longitude not between 120.7917034 and 121.1350232);`;
  ...
  await database.query(`update public.ew_ingestion_runs set status = 'completed', completed_at = now(), records_received = 29018, records_valid = 26459, records_rejected = 1, metadata = jsonb_build_object('staging_valid', 26459, 'staging_review', 2558, 'staging_rejected', 1, 'staging_inserted', 29018) where id = ${sqlLiteral(runId)} and source_id = ${sqlLiteral(SOURCE_ID)};`);
}
...
export async function runFoursquareLoad(args: readonly string[]): Promise<void> {
  ...
  await readReport(runId); const batches = await batchPaths(runId); const state = await loadState(runId); const database = await connectSupabaseLoaderDatabase();
  try {
  ...
  await ensureRun(runId, database);
  ...
  await reconcileRun(runId, database);
