...
const base: Omit<PriceEvidence, "id" | "minAmountMinor" | "maxAmountMinor"> = {
  ...
  pricePrecision: "exact", confidenceLevel: "VERIFIED", lastVerifiedAt: "2026-09-03T00:00:00.000Z",
};
const resolved = (evidence: PriceEvidence) => ({ evidence, inheritedFromChain: false } as const);
...
test("Phase 1 pilot budget examples retain conservative price semantics", () => {
  ...
  const distinctPassEnvelope = { ...base, id: "simulation-only", minAmountMinor: 55_000, maxAmountMinor: 95_000, pricePrecision: "derived" as const, confidenceLevel: "HIGH" as const };
